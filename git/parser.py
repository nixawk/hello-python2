...
def parse(filename, pretty=True):
    with open(filename, "rb") as o:
        ...
        for n in range(index["entries"]):
            ...
            # 12-bit name length,
            # if the length is less than 0xFFF (else, 0xFFF)
            ...
            if entry["extended"] and (index["version"] == 3):
                ...
                # 1-bit skip-worktree
                entry["skip-worktree"] = bool(
                    entry["extra-flags"] & (0b01000000 << 8))
                # 1-bit intent-to-add
                entry["intent-to-add"] = bool(
                    entry["extra-flags"] & (0b00100000 << 8))
                # 13-bits unused
